...
def main() -> None:
    ...
    for key in args.keys:
        ...
        # ---- ヘッド単品比率＝「無言のヘッド単品」が出やすい機種かの事前警告 ----
        # 2026-08-27の教訓: テキスト検出を完全にすり抜けるヘッド単品出品
        # （AIテンプレで「付属品: ヘッドカバー」とだけ書いて完品を名乗る形）は、
        # **実売のヘッド単品比率が高い機種ほど出やすい**。
        #   TSi3 54% → 8/27に2.04倍の偽候補  ／ G425 MAX 42%
        #   BRNR Mini 12% → テキスト検出でほぼ足りる
        hr = sum(1 for r in got if r["head_only"]) / len(got)
        if hr >= 0.40:
            print(f"  🔴 ヘッド単品比率 {hr*100:.0f}% — **写真でホーゼル確認を必須**に。"
                  f"無言のヘッド単品が出やすい機種")
        elif hr >= 0.20:
            print(f"  🟡 ヘッド単品比率 {hr*100:.0f}% — 写真確認を推奨")
        else:
            print(f"  ヘッド単品比率 {hr*100:.0f}%")
