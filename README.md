...
**①中古価格の平均 ②最安値 ③フリマ実売平均**を集計するWebアプリ。
...
| ③フリマ実売平均 | Yahoo!オークション 落札相場＋Yahoo!フリマ（実際に売れた価格） | ✅ 実装済み |
| 差額（買→売） | ①中古ショップ平均 − ③フリマ実売平均 を自動計算 | ✅ 実装済み |
...
- 試算 = フリマ実売平均 × 0.9 − 送料1700 − 中古最安値
- 集計はキーワード方式（楽天=中古／Yahoo=実売）＋圧縮キー照合で派生モデルを分離
